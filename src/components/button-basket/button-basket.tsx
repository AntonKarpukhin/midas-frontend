...
const ButtonBasket = ({className, addBasketProduct, name, count, ...props}: ButtonBasketProps) => {
	return (
		name && count &&
        <button className={cn(styles.buttonBasket, className)} type='button' {...props} onClick={() => addBasketProduct(name, count)}>
